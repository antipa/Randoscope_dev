...
uni_file = [pth,'\psf_uni_ds.mat'];
in_uni = load(uni_file);
psf_stack_uni = permute((in_uni.psf_noaber_uni_mid_ds), [2 3 1]);

rando_file = [pth,'\psf_aber_ds.mat'];
in_rando = load(rando_file);
%psf_stack_rando = permute(in.psf_montebest_ds, [2 3 1]);
psf_stack_rando = permute((in_rando.psf_aber_ds), [2 3 1]);

px = 4.541;  %Pixel size in microns/pixel in sensor space
Mag = 5.2;   %Magnification
px_obj = px/Mag;   %Object space microns/pixel
[Ny,Nx,Nz] = size(psf_stack_rando);
dz = 5;   %Slice spacing in microns (object space)
%%
third_psf = 'gaussian'
switch lower(third_psf)
    case('regular')
        reg_file = [pth,'\psf_reg_ds.mat'];

        in = load(reg_file);
        psf_stack = permute((in.psf_noaber_reg_mid_ds), [2 3 1]);
    case('gaussian')
        reg_file = [pth,'\diffuser_f=10_ALL_512x512.mat']
        in = load(reg_file);
        
        dz_g = mean(diff(in.zgrid));
        zmin = 0;
        zmax = dz * Nz;
        zmax_px_g = 100;%round(zmax/dz_g/1e3);
        psf_stack = in.zstack(:,:,1:zmax_px_g);
        
        psf_permute = permute(psf_stack,[1 3 2]);   %Resize along dim 2
        psf_resize = imresize(psf_permute,[Ny, Nz]);
        psf_stack = permute(psf_resize,[1 3 2]);
        % resample gaussian to have correct z spacing
end
...
hold off
%%
outpth = 'D:\\Randoscope\\dataforrebuttal\\correct_heigh_maps\\'
vec = @(x)x(:);
...
surfs_in = load(['D:\Randoscope\dataforrebuttal\newpsf\AllSurfaces_07152020.mat']);
surf_worst_in = load(['D:\Randoscope\dataforrebuttal\newpsf\WorstSurface.mat']);
...
c2 = r2
make_outim = @(im,cmap,nm)ind2rgb(gray2ind(uint8(im(r1:r2,c1:c2)/nm*255),length(colormap(cmap))),colormap(cmap));
surfreg = make_outim(imresize(surfs_in.Tunifocaluniform,[1024,1024]),'parula',smax);
surfuni = make_outim(imresize(surfs_in.Tunifocalnonuniform,[1024,1024]),'parula',smax);
%surfrand = make_outim(imresize(surfs_in.Tnoaber_multi_mid,[1024,1024]),'parula',smax);
surfopt = make_outim(imresize(surfs_in.Toptimized,[1024,1024]),'parula',smax);
surfworst = make_outim(imresize(surf_worst_in.Tworst_init,[1024,1024]),'parula',smax);
abersopt = make_outim(imresize(3e-3+surfs_in.Toptimized - surfs_in.Tmultifocalnonuniform,[1024,1024]),'parula',smax);
imshow(surfrand)
...
% imwrite(surfworst,[outpth,sprintf('worst_surface_%g_um_FoV_cmax_%g_um.png',(r2-r1)*px_srf*2,smax*1e3)])
% imwrite(abersopt,[outpth,sprintf('optimized_abers_%g_um_FoV_cmax_%g_cmin_%g.png',(r2-r1)*px_srf*2,1/2*smax*1e3,1/2*smax*1e3)])
imagesc(surfworst)
...
cmax = .04
cnorm = max(max(max(corrmat_uni(:)),max(corrmat_rando(:))),max(corrmat_regular(:)));
cmax = 1
J = customcolormap_preset('orange-white-purple');

imagesc_normalize = @(x)imagesc(x/max(x(:)));

subplot(2,3,1)
imagesc_normalize(axmat_uni)
axis image
caxis([0 cmax])
title('uni')

subplot(2,3,2)
imagesc_normalize(axmat_regular)
axis image
caxis([0 cmax])
title('regular')

subplot(2,3,3)
imagesc_normalize(axmat_rando)
axis image
caxis([0 cmax])
title('rando')

subplot(2,3,4)
imagesc_normalize(corrmat_uni)
axis image
caxis([0 cmax])
title('uni maxcorr')

subplot(2,3,5)
imagesc_normalize(corrmat_regular)
axis image
caxis([0 cmax])
title('regular maxcorr')

subplot(2,3,6)
imagesc_normalize(corrmat_rando)
axis image
caxis([0 cmax])
title('rando maxcorr')
drawnow
...
%% Prepare image files and save out
dtstamp = datestr(datetime('now'),'YYYYmmDD_hhMMss');

%%

cbarim = make_colorbar_im(J,10);

imnormalize = @(x)x/max(x(:));
corr_rando_out = gray2imagejfire(imnormalize(corrmat_rando),J);
corr_reg_out = gray2imagejfire(imnormalize(corrmat_regular),J);
corr_uni_out = gray2imagejfire(imnormalize(corrmat_uni),J);
key_dict = {'original file','z spacing','max value'}
opt_out = containers.Map(key_dict,{rando_file,dz,cmax});
reg_out = containers.Map(key_dict,{reg_file,dz,cmax});
badrand_out = containers.Map(key_dict,{uni_file,dz,cmax});

bad_file = [pth,'\',dtstamp,'_unifocal_coherence'];
opt_file = [pth,'\',dtstamp,'_optimized_coherence'];
reg_file = [pth,'\',dtstamp,'_regular_coherence'];

imwrite(corr_uni_out,[bad_file,'.png'])
containers2file([bad_file,'.txt'],badrand_out);

imwrite(corr_reg_out,[reg_file,'.png']);
containers2file([reg_file,'.txt'],reg_out);

imwrite(corr_rando_out,[opt_file,'.png']);
containers2file([opt_file,'.txt'],opt_out);

imwrite(cbarim,[pth,'\',dtstamp,'_colorbar.png'])
