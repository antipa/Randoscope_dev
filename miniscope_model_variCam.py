...
class Model(tf.keras.Model):
    def __init__(self,target_res=0.005,lenslet_CA=0.165,zsampling = 'uniform_random', cross_corr_norm = 'log_sum_exp', aberrations = False,GrinAber=[]):   #'log_sum_exp'
        ...
        target_option = 'airy'
        self.samples = (800,800)  #Grid for PSF simulation
        ...
        t = np.linspace(self.fmin, self.fmax, 20)    #Distance to sensor from mask in mm
        self.t=np.array(t).astype('float32')
        ...
        [xpos,ypos, rlist]=poissonsampling_circular(self,position_mode='initial_ballbearings',radius_mode='initial_ballbearings')
        ...
        self.target_res = tf.constant(target_res*M,dtype=tf.float32)# micron  
        ...
        D=1.22*self.lam/(tf.sin(2*tf.atan(self.target_res/(2.*self.t[1]))))  #0.514 for half_max, 1.22 for first zero. 
        x_airy=self.k*D*tf.sin(tf.atan(self.xgm/self.t[1]))/2.
        y_airy=self.k*D*tf.sin(tf.atan(self.ygm/self.t[1]))/2.
        ...
        sig_aprox=0.42*self.lam*self.t[1]/D
    ...
    def call(self, defocus_list):
        ...
        # Get psf stack
        zstack = self.gen_psf_stack(T, aper, 0)
    ...
    def gen_psf_stack(self, T, aper, prop_pad, zplanes=0):
        ...
        #else:
        #    zplanes = zplanes_opt   
        for z in range(0, len(self.t)):
            
           # zstack.append(gen_psf_ag_tf(T,self,zplanes[z],'angle',0., prop_pad,Grin=self.Grin[z]))
            zstack.append(gen_psf_ag_tf(T,self,zplanes,self.t[z],obj_def='angle'))
