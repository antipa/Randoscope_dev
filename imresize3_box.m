function Vnew = imresize3_box(Volume,newSize)
...
for ii = 1 : curSize(3)
    tempResult(:,:,ii) = imresize(Volume(:,:,ii),[newSize(1) newSize(2)],'box');
end
% Now resize X,Z
Vnew = zeros(newSize);
for ii = 1 : newSize(2)
    Vnew(:,ii,:) = imresize(squeeze(tempResult(:,ii,:)),[newSize(1) newSize(3)],'box');
end
end