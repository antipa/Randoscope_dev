...
%%
[gtfilename,gtpath] = uigetfile('D:\Randoscope\dataforrebuttal\newpsf\test_volumes\');
gtfile = [gtpath,gtfilename];
...
design_list = {'reg','worst_rand','opt','uni','gt'};
...
for n = 1:numel(design_list)
    ...
    switch lower(design_list{n})
        ...
        case('gt')
            xhat=gt.stackout*1000;
            from_file = gtfile;
            cur_psnr = 'N/A';
            switch lower(obj_type)
                case('usaf')
                    file_deets('bar spacing z um')=gt.bardepth_um;
                    file_deets('bar spacing x um')=gt.barwidth_um;
                    file_deets('x start um')=gt.xstart_um;
                    file_deets('z start um')=gt.zstart_um;
                case('dots')
                    file_deets('dot spacing z um')=gt.zlocs;
                    file_deets('dot spacing x um')=gt.xlocs;
