...
for zd = 9
    ...
    h = single(imresize(squeeze(h_permute(:,:,params.z_range,1:params.rank)),1/params.ds_psf,'box'));
    weights = single(imresize(squeeze(weights_permute(:,:,params.z_range,1:params.rank)),1/params.ds_psf,'box'));
    ...
    for tiff_slice = 101:150
        ...
        params.tau1 = gpuArray(.4e-7);   %.000005 works pretty well for v1 camera, .0002 for v2
        ...
        save_str = ['../recons/',date_string,'_',meas_name(1:end-4),'_',tiff_string];
        ...
        for n= 1:size(imout,3)
            out_names{n} = [filebase,'_',sprintf('%.3i',n),'_',tiff_string,'.png'];
        ...
        save([full_path,'/',meas_name(1:end-4),'_',date_string,'_',tiff_string,'.mat'], 'tau_iso','TVpars','xhat_out', 'options', 'comps_path','weights_path', 'b','params')
