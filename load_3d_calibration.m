...
for n = 1:length(psf_files_sorted)
    ...
    if mod(n,Nxy)==1
        zcount = zcount+1;
        psfs(zcount).xystack = zeros(Ny,Nx,Nc,Nxy);
        xycount = 0;
    ...
    psfs(zcount).xystack(:,:,:,xycount) = psf_in;
    imagesc(psfs(zcount).xystack(:,:,:,xycount)/100), axis image
    title(sprintf('Image number %i, xyindex %i',n,xycount))    
    drawnow
    if xycount == Nxy
        % Average across color channels to make 3d stack
        stack_mono = squeeze(mean(double(psfs(zcount).xystack) - bck_files_sorted(2*zcount).bck,3));   %2 because there are 2 bck files per z plane
        icenter = 89;
        ref_im = mean(double(psfs(zcount).xystack(:,:,:,icenter) - bck_files_sorted(2*zcount-1).bck),3);
        
        [comps, weights_interp, weights,shifts,yi_reg_out] = Miniscope_svd_xy(stack_mono,ref_im,rnk,'boundary_condition','circular');
        if zcount == 1
            comps_out(:,:,:,1) = comps;
            weights_out(:,:,:,1) = weights_out;
        else
