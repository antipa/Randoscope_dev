...
def poissonsampling_circular(model,factor=1,position_mode='random',radius_mode='random'):
    EAperR=model.CA-model.lenslet_CA2
    ...
    if position_mode=='random':
        ...
        while i<model.Nlenslets:
            while j<model.Nlenslets:
                if i!=j:
                    ...
                    if d<factor*model.lenslet_CA2 or xpos[j]**2+ypos[j]**2>EAperR**2 or xpos[i]**2+ypos[i]**2>EAperR**2:
                        vx=np.random.rand()
                        vy=np.random.rand()
                        if vx>0.5:
                            directionx=1
       ...
       -0.12,  0.64,  0.29,  0.  ,  0.16, -0.09, -0.66, -0.69,  0.66])
    elif position_mode=='initial36':
        xpos=np.array([-0.26580325, -0.06366066,  0.11256312, -0.35805833,  0.2517374 ,
        ...
        -0.4791343 ,  0.01053008,  0.6325499 , -0.5390383 , -0.29866594,
         ...
         0.33323294])
        ypos=np.array([-0.6977384 , -0.6529909 , -0.7300359 , -0.5332535 , -0.5763825 ,
        ...
        -0.062824  , -0.06667694, -0.02145099,  0.0011606 ,  0.04696656,
         ...
         0.6615066 ])
    elif position_mode=='initial36ky':
        xpos=np.array([-0.26580325, -0.06366066,  0.11256312, -0.35805833,  0.2517374 ,
        -0.5418476 , -0.18386264,  0.00938037,  0.56105363,  0.07780365,
         0.43474033, -0.60816294, -0.4205868 , -0.14209591,  0.6896641 ,
         0.14249665,  0.4223998 ,  0.6410226 , -0.54922813, -0.02941888,
    ...
    if radius_mode=='initial':
       ...
       11.681522 ,  3.4255354,  3.843439 ,  3.3348835,  6.368465 ,
        4.2305207])
    elif radius_mode=='initial36':
        rlist=np.array([3.3631554, 3.1450868, 2.3369374, 2.8383534, 4.653887 , 3.0785487,
        4.01228  , 3.5261483, 4.971912 , 2.784049 , 2.4973657, 2.4552286,
        3.904618 , 4.807646 , 3.705745 , 4.2464542, 2.7317836, 3.802583 ,
        2.3750806, 2.5409746, 2.4144897, 2.8948185, 3.4427238, 3.2145648,
        3.6137166, 4.5096583, 2.9535758, 2.3      , 2.6329267, 4.3741   ,
        4.126047 , 2.6814442, 3.0147676, 5.1478   , 2.5861335, 3.2871816])
       ...
       6.123522 ])
    elif radius_mode=='initial_nano_40correct':
        rlist=np.array([3.1133113, 2.9889655, 5.1478   , 4.5252404, 4.037016 , 3.4746933,
        3.9309883, 2.3      , 2.3358953, 2.533601 , 3.6438806, 2.767868 ,
        2.7176118, 2.4506342, 3.248452 , 2.4914272, 2.9304445, 4.148922 ,
        2.5772274, 2.3729289, 3.734807 , 3.1794462, 4.976635 , 3.8303874,
        3.3205197, 2.874171 , 2.4111557, 3.3958576, 4.3924384, 4.267209 ,
        2.6223824, 4.666323 , 2.820018 , 2.669148 , 3.5572765, 3.0498714,
        4.8164864])
...
def make_lenslet_tf(model):
        ...
        for n in range(model.Nlenslets):
            sph1 = model.lenslet_offset[n]+tf.real(tf.sqrt(tf.square(model.rlist[n]) - tf.square((model.xgm-
                                                                          ...
                                                                          )-tf.square(model.mean_lenslet_CA[n])))
            T = tf.maximum(T,sph1)
        aper = tf.sqrt(model.xgm**2+model.ygm**2) <= model.CA
        return T,aper
    
def make_lenslet_tf_controlr(model,rlenslet):
        T = tf.zeros([model.samples[0],model.samples[1]])
        for n in range(model.Nlenslets):
            sph1 = model.lenslet_offset[n]+tf.real(tf.sqrt(tf.square(model.rlist[n]) - tf.square((model.xgm-
                                                                          model.xpos[n])) - tf.square((model.ygm-model.ypos[n]))))-tf.real(tf.sqrt(tf.square(model.rlist[n]
...
def propagate_field_freq_tf(model,U,padfrac=0):
    ...
    Hf = tf_exp(2.*np.pi*model.t/model.lam * tf.sqrt(1-tf.square(model.lam*Fxn) - tf.square(model.lam*Fyn)))# nick's
    #Hf=tf_exp(2.*np.pi*model.t/model.lam-np.pi*model.lam*model.t*(tf.square(Fyn)+tf.square(Fxn)))  #book's
...
def gen_psf_ag_tf(T,model,z_dis, obj_def, pupil_phase=0, prop_pad = 0,Grin=0):  #check negatives in exponents
    ...
    if obj_def is 'angle':
        ...
        if z_dis is 'inf':
           ...
           # U_in = tf_exp(model.k*(Grin-z_dis +z_dis*tf.sqrt(1+tf.square(model.ygm/z_dis)+tf.square(model.xgm/z_dis)) + ramp))
            #U_in = tf_exp(-tf.to_float(tf.sign(z_dis))*model.k*(Grin-tf.to_float(tf.abs(z_dis)) +tf.sqrt(tf.to_float(tf.square(z_dis))+tf.square(model.ygm)+tf.square(model.xgm)) + ramp))
...
def project_to_aper_keras(model):
   ...
   # print(lenslets_dist)
    dist_new = tf.minimum(lenslets_dist, model.CA-0.75-tf.sqrt(2*model.rlist*model.lenslet_offset-tf.square(model.lenslet_offset)))
...
def zernikecartesian(coefficient,x,y):
    ...
    ZW = Z2+Z3+Z5+Z6#+Z12+Z13+Z23+Z24
...
#     Z37 =  Z[35] * 3*(70*r**8-140*r**6+90*r**4-20*r**2+1)
    ...
    return ZW
def birdson_poission_sample(r1=150,r2=75):
    P2=np.ones((1,2))
    while np.shape(P2)!=(36,2):
        p=poisson_disc_samples(width=1800, height=1800, r=r1)
        P=np.asarray(p)

        #scatter(P[:,0],P[:,1])
        #plt.axis('image')
        P2=P[(P[:,0]-900)**2+(P[:,1]-900)**2<(900-r2)**2]/1000
    x=P2[:,0]-0.9
    y=P2[:,1]-0.9
    return x,y

def bridson_poisson_N(r1 = 150, r2=75,CA=900, W=1800,H=1800,Nlenslets=1e9):
    ...
    return p_circ_x[inds]/1000, p_circ_y[inds]/1000
    


def find_best_initialization(model, num_trials = 2000, save_results =False,name=[]):
    loss_list = []
    ...
    for i in range(0,num_trials):
        #[xnew,ynew, rnew]=poissonsampling_circular(model,position_mode='random',radius_mode='random')
        rnew=np.random.permutation(1/(np.linspace(1/model.Rmax, 1/model.Rmin, model.Nlenslets)))
        xnew=[]
        while len(xnew)<model.Nlenslets:
            [xnew,ynew]=bridson_poisson_N(H=1800,W=1800,Nlenslets=36,CA=900,r1 = 160, r2 = 75)
    ...
    if save_results == True:
        ...
        scipy.io.savemat('/home/kyrollos/randoscope/BestAndWorst/worst_init'+name+'.mat', dict_worst)
    return x_best,y_best,r_best
